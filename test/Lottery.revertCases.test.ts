...
describe("Lottery Unit Tests", async function () {
    ...
    describe("10 player failure", async () => {
        it("Should revert with message", async function () {
            ...
            await expect(Lottery.buyTickets(player.address, amount, {gasLimit: 2000000})).to.be.revertedWith(revertedWith);
        ...
        it("Should revert with message #2", async function () {
            ...
            await approveTx.wait();
            await expect(Lottery.buyTickets(player.address, dollar(1), {gasLimit: 2000000})).to.not.be.revertedWith(revertedWith);
            ...
            await expect(Lottery.buyTickets(player2.address, amount, {gasLimit: 2000000})).to.be.revertedWith(revertedWith);
        ...
        it("Should revert with message #3", async function () {
            ...
            for (let i = 0; i < players.length; i++) {
                ...
                if (i == amountTooMuch.length - 1) {
                    await expect(Lottery.buyTickets(player.address, amount, {gasLimit: 20000000})).to.be.revertedWith(TooMuchAmount);
                } else {
                    await expect(Lottery.buyTickets(player.address, amount, {gasLimit: 20000000})).to.not.be.revertedWith(TooMuchAmount);
                }
            }
        })
    })
